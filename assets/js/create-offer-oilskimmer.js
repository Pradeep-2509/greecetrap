...
function buildProductOptions(select) {
  const catalogue = OIL_SKIMMER_CATALOGUE;
  select.innerHTML = '';
...
function recalcRow(row) {
  ...
  const unitPrice = getUnitPrice(productValue, material, 'Oil Skimmer');
...
document.getElementById('offerForm').addEventListener('submit', async (e) => {
  ...
  const items = Array.from(rows).map((row) => {
    ...
    const product = findProduct(selectedValue, 'Oil Skimmer');
    const material = row.querySelector('.material-select').value;
    ...
    const beltSize = row.querySelector('.belt-size').value.trim();
    const beltNumber = row.querySelector('.belt-number').value.trim();
    const beltPully = row.querySelector('.belt-pully').value.trim();

    const frameMaterial = row.querySelector('.frame-material').value.trim();

    // Build structured description for PDF rendering
    const descriptionParts = {
      motor: { capacity: motorCapacity, make: motorMake, rpm: motorRpm, type: motorType, phase: motorPhase },
      gearbox: { make: gearboxMake, ratio: gearboxRatio, type: gearboxType, bodySize: gearboxBodysize, shaft: gearboxShaft },
      belt: { size: beltSize, number: beltNumber, pully: beltPully },
      frame: frameMaterial
    };

    // Build readable description string as fallback
    const descLines = [];
    descLines.push(product ? product.capacity : selectedValue);
    if (motorCapacity || motorMake || motorRpm) {
      descLines.push('1. Motor');
      descLines.push(`Capacity : ${motorCapacity}, Make: ${motorMake}`);
      descLines.push(`RPM : ${motorRpm},Type : ${motorType}, Phase: ${motorPhase}`);
    }
    if (gearboxMake || gearboxRatio) {
      descLines.push('2. Gear Box');
      descLines.push(`Make : ${gearboxMake}`);
      descLines.push(`Ratio : ${gearboxRatio},Type : ${gearboxType}, Body size: ${gearboxBodysize}`);
      descLines.push(`Output Shaft:${gearboxShaft}`);
    }
    if (beltSize || beltNumber) {
      descLines.push('3. Belt');
      descLines.push(`Belt Size: ${beltSize}`);
      descLines.push(`Number of Belt: ${beltNumber}`);
      descLines.push(`Bottom & Top Pully : ${beltPully}`);
    }
    if (frameMaterial) {
